...
public class SwitcherAnimationLayer : MonoBehaviour
{
    ...
    private TypeAnimationLayer _defaultMoverAnimationLayer;
    private TypeAnimationLayer _moverAnimationLayer;

    private void Awake()
    {
        _defaultMoverAnimationLayer = TypeAnimationLayer.Default;
    }

    public void SetDefaultMoverAnimationLayer(TypeAnimationLayer moverAnimationLayer)
    {
        _defaultMoverAnimationLayer = moverAnimationLayer;
    }

    public void SetMoverAnimationLayer(TypeAnimationLayer moverAnimationLayer, float pathTime = 0f)
    {
        if (_moverAnimationLayer == moverAnimationLayer)
            return;

        SetNewAnimationLayer(_moverAnimationLayer, moverAnimationLayer, pathTime);
        _moverAnimationLayer = moverAnimationLayer;
    }

    public void ApplyDefaultMoverAnimationLayer(float pathTime = 0f)
    {
        SetMoverAnimationLayer(_defaultMoverAnimationLayer, pathTime);
    }

    public int GetIndexCurrentMoverAnimationLayer()
    {
        string name = _moverAnimationLayer.ToString();
